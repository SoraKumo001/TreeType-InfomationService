...
export class Params extends Module {
  public async onCreateModule() {
    ...
    if (remoteDB) {
      remoteDB.addEventListener("connect", () => {
        remoteDB.run(
          "create table IF NOT EXISTS params(params_name TEXT primary key,params_value json);"
        );
      });
    }
    return true;
  ...
  public async getParam(name: string) {
    ...
    return (await remoteDB.get2(
      "select params_value as value from params where params_name=$1",
      name
    )) as unknown;
  }
  public async setParam(name: string, value: unknown) {
    ...
    const result = await remoteDB.run(
      "insert into params values($1,$2) ON CONFLICT ON CONSTRAINT params_pkey do update set params_value=$2",
      name,
      value
    );
    return result;
